...
def get_social_stats_category():
    categories = (
        pd.read_csv(CLENSED_PATH / "social_stats.csv")["category"]
        .drop_duplicates()
        .sort_values()
    )
    for i in categories:
...
def get_social_stats_features(category: str):
    ...
    features = df[df["category"] == category]["feature"].drop_duplicates().sort_values()
    for i in features:
        yield i
...
def get_social_stats_years(feature: str):
    df = pd.read_csv(CLENSED_PATH / "social_stats.csv")
    years = df[df["feature"] == feature]["year"].drop_duplicates().sort_values()
    for i in years:
        yield i
...
def get_social_stats_years(feature: str):
    df = pd.read_csv(CLENSED_PATH / "social_stats.csv")
    area = df[df["feature"] == feature]["area"].drop_duplicates().sort_values()
    for i in area:
        yield i
...
def get_social_stats_values(feature: str, area: str):
    df = pd.read_csv(CLENSED_PATH / "social_stats.csv")
    df = (
        df[(df["feature"] == feature) & (df["area"] == area)][["year", "value"]]
        .drop_duplicates()
        .set_index("year")
        .sort_index()
    )
